...
def generate():
    ...
    generator.generate(prompt=enhanced, output_path=audio_file, num_inference_steps=steps, audio_length_in_s=audio_len)

    poas = float(evaluator.evaluate_clap(enhanced, audio_file))

    record = dict(id=run_id, domain=domain, original_prompt=prompt, enhanced_prompt=enhanced, poas_score=poas, audio_file=audio_file, rag_used=use_rag)
...
@app.route("/")
def dashboard():
    results_path = "outputs/results_table.csv"
    transfer_path = "outputs/domain_transfer_results.csv"
    ...
    # Use saved aggregate metrics directly
    if os.path.exists(transfer_path):
        transfer_df = pd.read_csv(transfer_path)
    else:
        transfer_df = None

    per_sample = df["poas_score"].tolist()

    # Remove GLOBAL row for charts
    domain_rows = transfer_df[transfer_df["domain"] != "GLOBAL"]

    global_row = transfer_df[transfer_df["domain"] == "GLOBAL"].iloc[0]

    best_domain_row = domain_rows.loc[domain_rows["mean_poas"].idxmax()]

    return render_template(
        "dashboard.html",
        global_poas=round(global_row["mean_poas"], 4),
        cri_variance=round(global_row["std_poas"], 4),
        best_domain=best_domain_row["domain"].capitalize(),
        best_score=round(best_domain_row["mean_poas"], 4),
        total_samples=int(global_row["samples"]),
        per_sample=per_sample,
        domain_means=domain_rows["mean_poas"].tolist(),
        domain_stds=domain_rows["std_poas"].tolist(),
        transfer_table=domain_rows.to_dict(orient="records")
    )
